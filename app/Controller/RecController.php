...
// 		}
	...
	public function ajax_delete(){
		...
		if($eliminate_flg == 0){
			...
			$this->Rec->eliminateFiles($ent['id'], 'img_fn', $ent); // ファイル抹消（他のレコードが保持しているファイルは抹消対象外）
	...
	*/
	...
	private function initCrudBase(){
		...
		$this->kensakuJoken=array(
			...
			array('name'=>'kj_file_name','def'=>null),
		...
		$this->kjs_validate=array(
				...
				'kj_rec_date1'=> array(
						'rule' => array( 'date', 'ymd'),
						'message' => '記録日付【範囲1】は日付形式【yyyy-mm-dd】で入力してください。',
						'allowEmpty' => true
				),
				'kj_rec_date2'=> array(
						'rule' => array( 'date', 'ymd'),
						'message' => '記録日付【範囲2】は日付形式【yyyy-mm-dd】で入力してください。',
						'allowEmpty' => true
				),
				'kj_note'=> array(
						'maxLength'=>array(
								'rule' => array('maxLength', 255),
								'message' => 'ノートは0文字以内で入力してください',
								'allowEmpty' => true
						),
				),
				'kj_rec_ctg_id' => array(
				...
				'kj_file_name'=> array(
						'maxLength'=>array(
								'rule' => array('maxLength', 255),
								'message' => 'ファイル名は512文字以内で入力してください',
								'allowEmpty' => true
						),
				),
				'kj_img_dp'=> array(
