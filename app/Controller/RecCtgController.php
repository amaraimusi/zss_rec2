 ...
 * 
 * @note
 * 記録カテゴリ画面では記録カテゴリ一覧を検索閲覧、編集など多くのことができます。
 * 
 * @date 2015-9-16 | 2018-10-4 フロントAページ追加
 * @version 3.2.0
 *
 */
class RecCtgController extends CrudBaseController {

	/// 名称コード
	public $name = 'RecCtg';
	
	/// 使用しているモデル[CakePHPの機能]
	public $uses = array('RecCtg','CrudBase');
	
	/// オリジナルヘルパーの登録[CakePHPの機能]
	public $helpers = array('CrudBase');

	/// デフォルトの並び替え対象フィールド
	public $defSortFeild='RecCtg.sort_no';
	...
	public function index() {
		...
		// CrudBase共通処理（前）
		$crudBaseData = $this->indexBefore('RecCtg');//indexアクションの共通先処理(CrudBaseController)
		
		//一覧データを取得
		$data = $this->RecCtg->findData($crudBaseData);
	...
	public function front_a(){
		...
		// フロントA用のコンポーネント
		$this->RecCtgFrontA = $this->Components->load('RecCtgFrontA');
		...
		$crudBaseData = $this->indexBefore('RecCtg',$option);//indexアクションの共通先処理(CrudBaseController)
		
		// ディレクトリパステンプレートを調整する(パスはindex用の相対パスになっているのでズレを調整しなければならない）
		$crudBaseData['dptData'] = $this->RecCtgFrontA->adjustDpt($crudBaseData['dptData']);
	...
	public function detail() {
		...
		$res=$this->edit_before('RecCtg');
		$ent=$res['ent'];
	

		$this->set(array(
				'title_for_layout'=>'記録カテゴリ・詳細',
				'ent'=>$ent,
		));
	...
	public function ajax_reg(){
		...
		// エンティティをDB保存
		$this->RecCtg->begin();
		$ent = $this->RecCtg->saveEntity($ent,$regParam);
		$this->RecCtg->commit();//コミット
	...
	public function ajax_delete(){
		...
		$this->RecCtg->begin();
		if($eliminate_flg == 0){
			$ent = $this->RecCtg->saveEntity($ent,$regParam); // 更新
		}else{
			$dtpData = $this->getDptData(); // ディレクトリパステンプレート情報
			$this->RecCtg->eliminateFiles($ent['id'],'img_fn',$dtpData); // ファイル抹消（他のレコードが保持しているファイルは抹消対象外）
			$this->RecCtg->delete($ent['id']); // 削除
		}
		$this->RecCtg->commit();//コミット
	...
	*/
	public function auto_save(){
		...
		// データ保存
		$this->RecCtg->begin();
		$this->RecCtg->saveAll($data); // まとめて保存。内部でSQLサニタイズされる。
		$this->RecCtg->commit();
	...
	public function csv_fu(){
		...
		$this->csv_fu_base($this->RecCtg,array('id','rec_ctg_val','rec_ctg_name','rec_ctg_date','rec_ctg_group','rec_ctg_dt','img_fn','note','sort_no'));
	...
	private function getDataForDownload(){
		...
		//セッションから検索条件情報を取得
		$kjs=$this->Session->read('rec_ctg_kjs');
		
		// セッションからページネーション情報を取得
		$pages = $this->Session->read('rec_ctg_pages');
	...
	private function initCrudBase(){
		...
		$this->kensakuJoken=array(
			...
			array('name'=>'kj_rec_ctg_name','def'=>null),
		...
		$this->kjs_validate=array(
				...
				'kj_rec_ctg_name'=> array(
						'maxLength'=>array(
								'rule' => array('maxLength', 255),
								'message' => '記録カテゴリは255文字以内で入力してください',
								'allowEmpty' => true
						),
				),
				'kj_note'=> array(
		...
		$this->field_data = array('def'=>array(
			...
			'id'=>array(
					'name'=>'ID',//HTMLテーブルの列名
					'row_order'=>'RecCtg.id',//SQLでの並び替えコード
					'clm_show'=>1,//デフォルト列表示 0:非表示 1:表示
			),
			'rec_ctg_name'=>array(
					'name'=>'記録カテゴリ',
					'row_order'=>'RecCtg.rec_ctg_name',
					'clm_show'=>1,
			),
			'note'=>array(
					'name'=>'備考',
					'row_order'=>'RecCtg.note',
					'clm_show'=>1,
			),
			'sort_no'=>array(
					'name'=>'順番',
					'row_order'=>'RecCtg.sort_no',
					'clm_show'=>0,
			),
			'delete_flg'=>array(
					'name'=>'無効フラグ',
					'row_order'=>'RecCtg.delete_flg',
					'clm_show'=>1,
			),
			'update_user'=>array(
					'name'=>'更新者',
					'row_order'=>'RecCtg.update_user',
					'clm_show'=>0,
			),
			'ip_addr'=>array(
					'name'=>'IPアドレス',
					'row_order'=>'RecCtg.ip_addr',
					'clm_show'=>0,
			),
			'created'=>array(
					'name'=>'生成日時',
					'row_order'=>'RecCtg.created',
					'clm_show'=>0,
			),
			'modified'=>array(
					'name'=>'更新日時',
					'row_order'=>'RecCtg.modified',
					'clm_show'=>0,
			),
