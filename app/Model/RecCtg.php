...
class RecCtg extends AppModel {
	 ...
	 * 記録カテゴリエンティティを取得
	 *
	 * 記録カテゴリテーブルからidに紐づくエンティティを取得します。
	 *
	 * @param int $id 記録カテゴリID
	 * @return array 記録カテゴリエンティティ
	 */
	...
	private function dumpSql($option){
		...
		$option['table']=$dbo->fullTableName($this->RecCtg);
		$option['alias']='RecCtg';
		
		$query = $dbo->buildStatement($option,$this->RecCtg);
	...
	private function createKjConditions($kjs){
		...
		if(!empty($kjs['kj_main'])){
			$cnds[]="CONCAT( IFNULL(RecCtg.rec_ctg_name, '') ,IFNULL(RecCtg.note, '')) LIKE '%{$kjs['kj_main']}%'";
		}
		
		// CBBXS-1003
		if(!empty($kjs['kj_id']) || $kjs['kj_id'] ==='0' || $kjs['kj_id'] ===0){
			$cnds[]="RecCtg.id = {$kjs['kj_id']}";
		}
		if(!empty($kjs['kj_rec_ctg_name'])){
			$cnds[]="RecCtg.rec_ctg_name LIKE '%{$kjs['kj_rec_ctg_name']}%'";
		}
		if(!empty($kjs['kj_note'])){
			$cnds[]="RecCtg.note LIKE '%{$kjs['kj_note']}%'";
		}
		if(!empty($kjs['kj_sort_no']) || $kjs['kj_sort_no'] ==='0' || $kjs['kj_sort_no'] ===0){
			$cnds[]="RecCtg.sort_no = {$kjs['kj_sort_no']}";
		}
		$kj_delete_flg = $kjs['kj_delete_flg'];
		if(!empty($kjs['kj_delete_flg']) || $kjs['kj_delete_flg'] ==='0' || $kjs['kj_delete_flg'] ===0){
			if($kjs['kj_delete_flg'] != -1){
			   $cnds[]="RecCtg.delete_flg = {$kjs['kj_delete_flg']}";
			}
		}
		if(!empty($kjs['kj_update_user'])){
			$cnds[]="RecCtg.update_user LIKE '%{$kjs['kj_update_user']}%'";
		}
		if(!empty($kjs['kj_ip_addr'])){
			$cnds[]="RecCtg.ip_addr LIKE '%{$kjs['kj_ip_addr']}%'";
		}
		if(!empty($kjs['kj_created'])){
			$kj_created=$kjs['kj_created'].' 00:00:00';
			$cnds[]="RecCtg.created >= '{$kj_created}'";
		}
		if(!empty($kjs['kj_modified'])){
			$kj_modified=$kjs['kj_modified'].' 00:00:00';
			$cnds[]="RecCtg.modified >= '{$kj_modified}'";
		}
	 ...
	 * エンティティをDB保存
	 *
	 * 記録カテゴリエンティティを記録カテゴリテーブルに保存します。
	 *
	 * @param array $ent 記録カテゴリエンティティ
	 * @param array $option オプション
	 *  - form_type フォーム種別  new_inp:新規入力 , copy:複製 , edit:編集
	 *  - ni_tr_place 新規入力追加場所フラグ 0:末尾 , 1:先頭
	 * @return array 記録カテゴリエンティティ（saveメソッドのレスポンス）
	 */
