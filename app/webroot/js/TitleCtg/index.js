...
$(function() {
	...
	$('#title_ctg_tbl').show();// 高速表示のためテーブルは最後に表示する
...
function init(){
	...
	crudBase = new CrudBase({
			'src_code':'title_ctg', // 画面コード（スネーク記法)
			'kjs':kjs,
			'ni_tr_place':1,
		});
	...
	pwms = new ProcessWithMultiSelection({
		'tbl_slt':'#title_ctg_tbl',
		'ajax_url':'title_ctg/ajax_pwms',
			});
	...
	// リアクト機能の初期化■■■□□□■■■□□□■■■□□□開発中
	crudBase.reactInit('title_ctg_tbl, hyo2');
	
	// CrudBase一括追加機能の初期化
	var today = new Date().toLocaleDateString();
	crudBase.crudBaseBulkAdd.init(
		[
			{'field':'title_ctg_name', 'inp_type':'textarea'}, 
//			{'field':'title_ctg_val', 'inp_type':'textarea'}, 
//			{'field':'title_ctg_group', 'inp_type':'select', 'list':title_ctgGroupList, 'def':2}, 
//			{'field':'title_ctg_date', 'inp_type':'date', 'def':today}, 
//			{'field':'note', 'inp_type':'text', 'def':'TEST'}, 
//			{'field':'sort_no', 'inp_type':'sort_no', 'def':1}, 
		],
		{
			'ajax_url':'title_ctg/bulk_reg',
			'ta_placeholder':"Excelからコピーしたタイトルカテゴリ名、タイトルカテゴリ数値を貼り付けてください。（タブ区切りテキスト）\n(例)\nタイトルカテゴリ名A\t100\nタイトルカテゴリ名B\t101\n",
		}
	);
}
...
function calendarViewKShow(){
	// カレンダービューを生成 
	crudBase.calendarViewCreate('title_ctg_date');
}
